...
<SequenceItemLayout index={index}>
<div class="graph">
  {#each graph.nodes() as node_id}
    ...
    >
      ...
      {:else}
        {node_id}
      {/if}
	  <div class="amino-acid amino-acid-{REPORT[node_id].amino_acids[index]}">
	  	{REPORT[node_id].amino_acids[index]}
	  </div>
    </a>
  {/each}
